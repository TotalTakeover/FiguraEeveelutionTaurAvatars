...
function events.RENDER(delta, context)
	if context == "FIRST_PERSON" or context == "RENDER" or (not client.isHudEnabled() and context ~= "MINECRAFT_GUI") then
		...
		if pose.stand or pose.crouch then
			...
			nameOffset = posOffset - vec(0, (pose.sleep and -14 or -2) * modelEyeHeight, pose.sleep and 0 or -20) / 16
			...
			nameOffset = vectors.rotateAroundAxis(-player:getRot().x - (pose.sleep and 0 or 90), nameOffset, vec(1, 0, 0))
